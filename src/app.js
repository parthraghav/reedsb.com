...
function format( template, object, delimiter = "{{}}" ) {
    ...
    while ( 0 <= template.indexOf(head) && template.indexOf(head) < template.length ) {
        ...
        template = template.slice(0, i ) + val + template.slice( j+head.length, template.length);
    }
...
const CONFIG = {
    ...
    authorised_users: ['raghavp@reed.edu','huques@reed.edu','dakshami@reed.edu','pifreeman@reed.edu', 'trouts@reed.edu', 'taqia@reed.edu'],
...
class SinApp {
    ...
    logout() {
        self = this;
        fb.auth().signOut().then( (function (self) {
            // Sign-out successful.
    ...
    refresh_view(user) {
        ...
        if (!(this.is_view("funding-poll")||this.is_view("senate-election"))){
            ...
            // put profile picture
            if (user) {
                const navProfilePhoto = document.querySelector('#user-nav-profile-photo');
                const navDisplayName = document.querySelector('#user-nav-display-name');
            ...
            var fp_timeout = (new Date().getTime() < CONFIG.fpopens && new Date().getTime() > CONFIG.fpends);
            if (fp_timeout) {
            ...
            if (user){
                $('#sin-fp-name').text( this.auth.currentUser.displayName.replace(/ .*/,'') );
            ...
            var se_timeout = (new Date().getTime() > CONFIG.seopens && new Date().getTime() > CONFIG.seends);
            if (se_timeout) {
            ...
            if (user){
                $('#sin-se-name').text( this.auth.currentUser.displayName.replace(/ .*/,'') );
        ...
        // PAGES
        var logout_alert, loggedin_form, timeout_alert, loggedin_viewer, auth_alert;
        ...
        if (this.is_view("register-club")) {
            ...
            if (user) {
                if (new Date().getTime() < CONFIG.rcopens && new Date().getTime() > CONFIG.rcends) {
        ...
        //request-funds
            ...
            if (user) {
                if (new Date().getTime() < CONFIG.rfsopens && new Date().getTime() > CONFIG.rfsends) {
                    timeout_alert[0].classList.remove('d-none');
                    timeout_alert[0].classList.add('d-block');
    ...
    is_view(view_name) {
        return (this.DEBUG_MODE && this.view == view_name) || (!this.DEBUG_MODE && this.view == view_name+".php");
    }
    ...
    club_register() {
        ...
        this.db.collection("signatories").doc(CONFIG.semester).get().then(function(doc) {
            var Data = doc.data();
        ...
        function validate_form(signators, title, contact, about, identity_request, identity_mission, identity_community, identity_activity, identity_open, identity_outsidefunding) {

            // SIGNATORS VALIDATION
            if (signators.length != 0) {
                var j = signators.split(',').reduce( (acc, el) => acc && signatories.reduce( (acc, obj) => obj.Email == el ), 1 );
                var control_emails = signatories.map( el => el.Email ), acc_array = [];
                for (var i = 0; i < signators.split(',').length; i++) {
                    acc_array.push(control_emails.includes(signators.split(',')[i]));
                }
                if (signators.length > 0 && acc_array.reduce( (acc, el) => acc && el )) {
                    //console.log("valid");
                } else {
                    // signators error
            ...
            if (identity_request) {
                ...
                if ( !CONFIG.bypass_full_id_app && (200 > ic.length || ic.length > 400) ) {
                ...
                if ( !CONFIG.bypass_full_id_app && (100 > ia.length || ia.length > 300) ) {
                    alert(" Prompt III under or above word limit. ")
                    return false;
                }

                var io = identity_open.split(" ");
                if ( !CONFIG.bypass_full_id_app && (50 > io.length || io.length > 300) ) {
        ...
        $('#sin-rc-form-submit').click( function(e) {
            ...
            var identity_outsidefunding = $('#sin-rc-identity-outside-funding')[0].value;
            
            var valid_flag = validate_form( signators, title, contact, about, identity_request, identity_mission, identity_community, identity_activity, identity_open, identity_outsidefunding );
            ...
            if (valid_flag) {
                ...
                if (identity_request) {
                    ...
                    data.identity_community = identity_community == "" && CONFIG.bypass_full_id_app ? "Not submitted" : identity_community;
                    data.identity_activity = identity_activity == "" && CONFIG.bypass_full_id_app ? "Not submitted" : identity_activity;
                    data.identity_open = identity_open == "" && CONFIG.bypass_full_id_app ? "Not submitted" : identity_open;
                    data.identity_outsidefunding = identity_outsidefunding == "" && CONFIG.bypass_full_id_app ? "Not submitted" : identity_outsidefunding;
                }
                self.db.collection("registered_clubs").doc(CONFIG.semester).update({
                    registered_clubs: fb.firestore.FieldValue.arrayUnion(data)
                }).then(function(){
                    //console.log("write head", docRef.id);
                    alert("Your club registration request has been received. Thank you for applying.");
    ...
    fund_request() {
        ...
        var organisations = [];
        this.db.collection("registered_clubs").get().then(function(querySnapshot) {
            ...
            Util.make_organisation_selector( '#sin-rf-choose-organisation', organisations );
        });


        // SUBMISSION
        var possible_reserves = ["Identity Funds", "Student Body Funds", "Finance Committee"];
        var possible_purposes = ["Club/Organisation Funding","Community Event","Community Project"];
        var possible_categories = ['Refreshments','Administration','Entertainment','Capital Improvements','A/V','Miscellaneous'];            

        function validate_form(purpose, reserve, signators, orgs, description, budget, certifications) {

            if (!possible_purposes.includes(purpose)) {
                alert('Either you have not selected the "purpose" or tampered with the parameters. Please select the "purpose". The form remains unsubmitted.');
                return false;
            }

            if (!possible_reserves.includes(reserve)) {
                alert('Either you have not selected the "reserve" or tampered with the parameters. Please select the "reserve". The form remains unsubmitted.');
                return false;
            }
            ...
            if (description_count == 0 || description_count >= 150) {
                alert("Length of the purpose description should be greater than 0, but less than 150 words.");
                return false;
            }

            // BUDGET VALIDATION [{}]
            if (budget.length==0) {
                alert("There are no items in your budget. Add at least one item to continue.")
                return false;
            }
            for (var i = 0; i < budget.length; i++) {
                if ( !(0 < budget[i].item.split(" ").length < 50) ) {
                    alert("Item description of item "+(i+1)+" is either empty or longer than 50 words.");
                    return false;
                }
                if (!possible_categories.includes(budget[i].category)){
                    alert("Item category of item "+(i+1)+" is either not selected or invalid.");
                    return false;
                }
            ...
            if (!certifications[0]) {
                alert("Please complete the certification 1. The form remains unsubmitted.");
                return false;
            }
            if (!certifications[1]) {
                alert("Please complete the certification 2. The form remains unsubmitted.");
                return false;
            }
        ...
        function submit_fund_request(e) {
            ...
            var budget_selector = $('#sin-rf-budget-form>div:not(#sin-rf-item-header):not(#sin-rf-empty-cart):not(#sin-rf-item-footer)');
            ...
            budget_selector.each(function(){
                var obj = { 
                    ...
                    category: possible_categories[$(this).find('.sin-rf-item-category').val()-1],
                    amount: $(this).find('.sin-rf-item-cost').val()
                };
                data_budget.push(obj);

            });
            var data_certification = [ certification1_selector[0].checked, certification2_selector[0].checked ];
            
            valid_flag = validate_form(data_purpose, data_reserve, data_signator, data_organisation, data_description, data_budget, data_certification);
            if (valid_flag) {
                var data = {
                    ...
                    if (snap.exists) {
                        docRef.onSnapshot(function(doc){
                            docRef.update({
                                ...
                                alert("Your fund request was successfully submitted. Thank you!");
                                window.location.href = document.referrer;
                            }).catch(function(error){
                            if(self.DEBUG_MODE){
    ...
    funding_poll() {
        ...
        this.db.collection('users_who_have_voted').doc(CONFIG.poll_name+'-'+CONFIG.poll_id).get().then(function(doc){
            var data = doc.data();
            if (data == undefined){
                proceed();
                return;
    ...
    dash_door() {
        var club_viewer_el, club_viewer_body_el, budgets_viewer_el, budgets_viewer_body_el;
        ...
        $('#sin-dd-refresh').click(function(){
         ...
         var table, budget_table, clubs = [], fund_requests = [];
        ...
        "columnDefs": [ {
                 ...
                 { 
                    ...
                    "render": function(data) {
                        ...
                        data.split(",").forEach(function(el){
                            unsafe_html += get_pill( shuffled_colors[i % shuffled_colors.length ], el );
                            i += 1;
                        })
                        return unsafe_html;
                    }
            ...
            club_viewer_body_el.on('click', 'td.details-control', function () {
                 ...
                 $('.fin-dd-read-more').readmore({ speed: 75, lessLink: '<a href="#">Read less</a>' });
             });

             inner_table.on("user-select", function (e, dt, type, cell, originalEvent) {
                 if ($(cell.node()).hasClass("details-control")) {
                     e.preventDefault();
                 }
             });

            inner_table.on( 'order.dt search.dt', function () {
                inner_table.column(0, {search:'applied', order:'applied'}).nodes().each( function (cell, i) {
                    cell.innerHTML = i+1;
                } );
            } ).draw();
            ...
            budgets_viewer_body_el.on('click', 'td.details-control', function () {
             ...
             $(document).on('click','.shoot_email',function(){
                ...
                if (_amount == "" || parseInt(_amount) == 0) {
                    _main = encodeURIComponent(`We regret to inform you that the Senate has decided NOT to allocate any funds.\n\nOnwards,\nTreasury`)
                } else {
                    _main = encodeURIComponent(`Congratulations! You have been allocated $${_amount}/-.\n\nOnwards,\nTreasury`)
                }
                window.open($(this).attr('data-link') + _main, '_blank', 'location=yes,height=570,width=520,scrollbars=yes,status=yes');
             });
        ...
        if (window.localStorage.cached2 == "true") {
            table = return_clubs_data_table(JSON.parse(window.localStorage.getItem('fb_data')));
        } else {
            this.db.collection('registered_clubs').doc(CONFIG.semester).get().then(function(doc) {
        ...
        this.db.collection('fund_requests').doc(CONFIG.semester).get().then(function(doc) {
            var Data = doc.data();
            if (Data == undefined) {
                return;
            }
            ...
            reqs.forEach(function(Case){
                Case.total = Case.budget.reduce(function(a,b){
                    return { amount: Math.round(Number(parseFloat(a.amount).toFixed(2)) + Number(parseFloat(b.amount).toFixed(2))) };
                }).amount;
        ...
        this.db.collection('signatories').doc(CONFIG.semester).get().then(function(doc){
            ...
            Data['signatories'].filter(function(Case){
                return Case.Email!=undefined
            }).forEach(function(Case){
                $('#sList').append( $('<option>').text(Case.Name + ", " + Case.Email) );
        ...
        function AddSignatories(){
            ...
            if (aList.length == 0) {
                ...
                var res = Object.entries(cMap).map(function(entry){
                    return {'Name':entry[1],'Email':entry[0]}
                }).concat(nList);
                $('.alert-area').empty();
                
                self.db.collection('signatories').doc(CONFIG.semester).set({
                    signatories: res
                }).then(function(){
                ...
                var _listAlreadyStr = aList.map(arr=>"<li>"+arr[0]+", &lt;"+arr[1]+"&gt;</li>").join("");
    ...
    view_funding_poll_results() {
        ...
        if(!CONFIG.direct_server_load) {
            ...
            $.ajax({
                dataType: "json",
                url: "./data/" + CONFIG.poll_name + '-' + CONFIG.poll_id + '-results.json',
                data: {},
                success: function(response){
            ...
            this.db.collection('registered_clubs').where("poll_participate", "==", true).get().then(function(querySnapshot){
                querySnapshot.forEach(function(doc){
                    var data = doc.data();
        ...
        function show_results (data) {
            if(!Util.is_mobile()){
                ...
                var $img = $('<img>').attr({'src':'http://reedsb.com/data/mobile_results_fall2019_sb_fp.png','width':'100%'});
        ...
        $('#sin-fp-timeout-alert .jumbotron-alert').click(function(){
            window.location.href = "http://reedsb.com/request-funds";
        });
    ...
    run_for_senate() {
        ...
        name_input.attr("placeholder", this.unsafe_user.displayName).val(this.unsafe_user.displayName).focus().blur();

        // load senate positions
        var senate_positions = [];

        this.db.collection("elections").doc(CONFIG.secampaign).get().then(function(doc) {
            var data = doc.data();
            if (data == undefined) {
                return;
            }
            ...
            //console.log(signatories);
            Util.make_senate_position_selector( '#sin-rfs-form-position', senate_positions );
        });
        ...
        function campaign_text_editor() {
            ...
            campaign_text.bind('input propertychange', function(){
                ...
                var L = this.value.length, Max = CONFIG.campaign_max_length, Min = CONFIG.campaign_min_length;
                if(this.value.length>0){
                    campaign_progress.css('height','5px');
                    campaign_progress.find('div').css('width',parseInt(L*100/Max)+"%");
                    count_bar.show();
                ...
                if (L>Max||L<Min){
                    ...
                    if (L<Min){     
                        count_bar.text(`${L}/${Max} (You should write at least ${Min} characters.)`);
                    }
                    count_bar.show();
                } else {
        ...
        function validate_form(data) {
            ...
            if (data.campaign_msg.length>CONFIG.campaign_max_length||data.campaign_msg.length<CONFIG.campaign_min_length){
        ...
        function submit_nomination(e) {
            ...
            if (validate_form(data)) {
                 self.db.collection("nominations").doc(CONFIG.secampaign).update({
                    nominations: fb.firestore.FieldValue.arrayUnion(data)
                }).then(function(docRef){
                    var new_senate_positions;
                    if (senate_positions.find(el=>el.Role==data.senate_position)){
                    ...
                    self.db.collection("elections").doc(CONFIG.secampaign).set({
                        senate_positions: new_senate_positions
                    },{merge:true}).then(function(d){
                        alert("Your senate nomination was successfully submitted. Thank you!");                        
                        window.location.href = document.referrer;
                    }).catch(function(error){
                        if(self.DEBUG_MODE){
    ...
    senate_election() {
        var self = this;
        this.db.collection("users_who_have_voted").doc(CONFIG.secampaign).where("Email", "==", this.auth.currentUser.email).get().then(function(querySnapshot){
...
class SenateElectionViewer {
    ...
    _intro_screen() {
        var self = this;
        show(this.intro_screen), hide(this.vote_how_el), hide(this.vote_done_el), hide(this.vote_screen), hide(this.preloader_screen);
        this.app.db.collection('elections').get().then(function(querySnapshot) {
    ...
    proceed_step1() {
        show(this.vote_how_el), hide(this.intro_screen), hide(this.vote_done_el), hide(this.vote_screen), hide(this.preloader_screen);
    ...
    adjust_frame_size() {
        ...
        var comp1 = $('#sin-se-comp1'), comp2 = $('#sin-se-comp2');
        if (!this.app.mobile) {
            var w = $('body').width();
            if (760 < w && w < 1555) {
    ...
    proceed_step2() {
        show(this.preloader_screen), hide(this.vote_screen), hide(this.vote_how_el), hide(this.intro_screen), hide(this.vote_done_el);
        var self = this;
        ...
        this.app.db.collection('nominations').get().then(function(querySnapshot) {
            querySnapshot.forEach(function(request) {
                var request_Data = request.data();
                self.nominee_data.push(request_Data);
            });           
        }).then(function(){
            show(self.vote_screen), hide(self.vote_how_el), hide(self.intro_screen), hide(self.vote_done_el), hide(self.preloader_screen);
    ...
    submit_votes(){
        ...
        Object.keys(this.results).forEach(function(key){
            ...
            update_obj[`Results-${key.replace(/\s/g, '-')}`][String(self.results[key])] = fb.firestore.FieldValue.increment(1);
        });
    ...
    prepare_for_voting(data) {
        ...
        $( "#sin-se-submit-button" ).click(function(){
            ...
            var candidates = self.nominations_by_case[Object.keys(nominations_by_case)[self.voting_index]];
            ...
            self.results[Object.keys(nominations_by_case)[self.voting_index]] = result_str;
        ...
        var sortable = new Sortable(item_box[0], {
            ...
            onEnd: function (evt) {
                ...
                item_box.children('div').each(function(i){
                    $(this).find('.sin-se-rank').find('h1').text(i+1);
                });
    ...
    next_voting() {
        ...
        item_box = $('#sin-se-items'), self = this, i = 0, self = this,
        open_positions = Object.keys(this.nominations_by_case),
        curr_position = open_positions[this.voting_index];
        rearranger_topic.text(`(${this.voting_index+1}/${open_positions.length}) Running for ${curr_position}...`);
        item_box.empty();
        ...
        $('.sin-se-item').click(function(e){
            ...
            if (self.app.mobile) {
                ...
                var go_back_button = $('<a class="btn btn-warning go-back-btn">Go back</a>');
            ...
            e.preventDefault();
            var name = atob($(this).attr('data-checksum'));
            var corresponding_case = self.nominations_by_case[curr_position].filter( el => el.name == name );
            console.log(corresponding_case);
            if (corresponding_case.length != 0) {
                var user_data = corresponding_case[0];
                hide($('#sin-se-unopened-tab'));
                show($('#sin-se-loading-tab'));
                $('#sin-se-data-tab').html( format('<div class="container"><p><i>Why vote for</i></p><h1 class="display-4">{{name}}</h1><hr><p class="lead">{{msg}}</p><small class="text-light">Last updated on {{date}}.</small></div></div>',{
                    name: user_data.name,
                    msg: user_data.campaign_msg,
                    school_year: Util.get_worded_school_year(user_data.school_year),
                    date: new Date(parseInt(user_data.registered.seconds) * 1000).toISOString().substr(0, 10)
                }));
                setTimeout(function(){
    ...
    proceed_finish() {
        ...
        setTimeout(function(){
            ...
            self.submit_votes();
            setTimeout(function(){
                self.app.db.collection("users_who_have_voted_"+CONFIG.secampaign).add({
...
class FundingPollViewer {
    ...
    _constructor_intro() {
        ...
        this.app.db.collection("polls").doc(CONFIG.poll_name+'-'+CONFIG.poll_id).get().then( function(doc) {
            if (doc.exists) {
    ...
    _constructor_voting() {
        ...
        if(this.app.mobile) {
            instruction_modal.animate({scrollTop: jQuery(document).height() }, 2000);            
        }
        var registered_clubs = [], club_cards = [];

        function update_engagement() {
            var total_likes = 0, total_dislikes = 0, total_boosts = 0;
            ...
            if ( (total_likes+total_dislikes+total_boosts) < CONFIG.vote_submission_threshold * no_of_clubs ) {
...
("poll_participate", "==", true)
        this.app.db.collection("registered_clubs").doc(CONFIG.semester).get().then(function(doc) {
                if (!doc.exists) {
                    return;
                }
                ...
                clubs.forEach(function(club) {
                    club.id = uuidv4();
                    registered_clubs.push({ id: club.id, title: club.title, about: club.about, contact: club.contact, signators: club.signators, liked: false, disliked: false, boosted: false });
                    var callbacks = {};
                    var callback_dislike = function(){
                        ...
                        if (!pCard.hasClass('card-danger')){
                            ...
                            registered_clubs.find( el => el.id == club.id ).liked = false;
                            registered_clubs.find( el => el.id == club.id ).boosted = false;
                            registered_clubs.find( el => el.id == club.id ).disliked = false;
                        }
                        update_engagement();
                    }
                    ...
                    var callback_boost = function(){
                        ...
                        if (!pCard.hasClass('card-warning')){
                            if (parseInt(boost_counter.text()) < CONFIG.max_boosts) {
                                ...
                                registered_clubs.find( el => el.id == club.id ).liked = false;
                                registered_clubs.find( el => el.id == club.id ).disliked = false;
                                registered_clubs.find( el => el.id == club.id ).boosted = true;
                            } else {
                    ...
                    var club_card = Util.generate_club_card( club.id, club.title, club.signators.split(","), club.about, callback_like, callback_dislike, callback_boost );
        ...
        function poll_submit_fn() {
            function findClubIndex(dbData, el1){
                return dbData.findIndex( el2 => el2.title == el1.title && el2.signators == el1.signators );
            }
            var revisedContestantData = [];
            self.app.db.collection("registered_clubs").doc(CONFIG.semester).get().then(function(doc){
                if (!doc.exists){
                    return;
                }
                ...
                registered_clubs.forEach( function(club){
                    var delta_update, delta = club.liked || club.disliked || club.boosted;
                ...
                self.app.db.collection("registered_clubs").doc(CONFIG.semester).update({
                    registered_clubs: revisedContestantData
                }).then(function(){
                    console.log("vote committed");
                    self.app.db.collection("users_who_have_voted").doc(CONFIG.poll_name+'-'+CONFIG.poll_id).set({
                        voters: fb.firestore.FieldValue.arrayUnion({
                            Email: self.app.auth.currentUser.email,
                            Time: new Date().getTime()
                        })
                    }).then(function(){
                        self.app.db.collection("polls").doc(CONFIG.poll_name+'-'+CONFIG.poll_id).update({
                            'voter-turnout' : fb.firestore.FieldValue.increment(1),
                            'voters': fb.firestore.FieldValue.arrayUnion(self.app.auth.currentUser.email)
                        }).then(function(){
                            alert("your vote has been registered! Thank you for participating in the funding poll!");                
                            window.location.reload();                           
                        })
                    });
                });
        ...
        window.poll_submit_fn_legay = function() {
            ...
            registered_clubs.forEach( function(club){
                ...
                if (delta) {
                    ...
                    batch.set( club_collection.doc( club.id ), delta_update, { merge: true } );

                }

            } );

            batch.set(self.app.db.collection("polls").doc(CONFIG.poll_name+'-'+CONFIG.poll_id), { 'voter-turnout' : fb.firestore.FieldValue.increment(1) }, { merge:true } );
    ...
    switch(view) {
        ...
        function switch_helper(view) {
            if ( Object.values(this.views).map(el => $(el)[0]).includes( $(view)[0] ) ) {
                for ( const key in this.views ) {
                    var loop_view = $(this.views[key])[0], control_view = $(view)[0];
                    if ( control_view == loop_view ) {
                        control_view.classList.remove('d-none');
                        control_view.classList.add( 'd-' + control_view.getAttribute('data-display') );
                    } else {
                        loop_view.classList.remove( 'd-' + loop_view.getAttribute('data-display') );
        ...
        setTimeout( (function(fn, arg){ return fn.call(arg); })( switch_helper.bind(this), view ), this.lag/2 );
    ...
    async wait(time) {
        ...
        function timeout(ms, env) {
            return new Promise(resolve => setTimeout((function(){ return self }).bind(env), ms));
        }
...
class FundingPoll {
    ...
    init_intro_screen() {
        ...
        enter_button.onclick = (() => this.viewer.switch( this.viewer.views.voting )).bind(this);
...
class Util {
    ...
    static make_nominee_box(data, index) {
        ...
        year_short = year % 100,
        votes = parseInt(data.votes), vote_message;
        if (votes == 0) {
        ...
        return `
          <div class="m-1 grabbable sin-se-item row no-gutters" data-checksum=${btoa(data.name)}>
            ...
            <div class="col-10 col-sm-10 col-md-7 sin-se-details">
                <div class="card-body text-left">
                    ...
                    <p class="card-text sin-se-description mb-0">${TFIDF(data.campaign_msg)}</p>
            ...
            <div class="col-0 col-xs-0 col-sm-0 d-none d-sm-none d-xs-none col-md-3 container d-md-flex sin-se-photo">
                <img src="${data.photoURL}" class="card-img text-center justify-content-center align-self-center mx-0 mx-auto" alt="...">    
            </div>
    ...
    static congratulatory_confetti() {
        ...
        $(function() {
          ...
          var colorThemes = Util.shuffle(['#FF9800','#A52A2A','#5F9EA0','#FF7F50','#EF5350','#7E57C2','#2196F3','#00BCD4','#009688','#CDDC39','#808080','#FFC0CB','#9acd32','#ee82ee','#ff6347','#008080','#009688','#6a5acd','#4169e1']).map(function(el){
            return function() {
                return el;
            };
          });
    ...
    static visualise_results(data){
        ...
        var width = Math.max(500,window.innerWidth - 2*(margin.left + margin.right)),
            height = 1.5*window.outerHeight - margin.top - margin.bottom;

        var svg = d3.select("#sin-fp-results-graphic").append("svg")
    ...
    static format_budget_row(d){
         ...
         var unsafe_html = $('<table>').addClass('p-3', 'display').attr({
            "width":"80%",
         ...
         var unsafe_body = $('<tbody></tbody>'), cell, current;
         var i = 0;
         ...
         var link = 'mailto:'+d.signators+'?subject='+encodeURIComponent('Re: Fund Request')+'&body='+encodeURIComponent('Hi,\n\nYou requested $'+d.total+' from the Student Body Senate. ');
         var description_holder = $(`<div class="row exp-row"><div class="col-md-12 fin-dd-fopen-about p-3">${description}</div></div>`);
    ...
    static format_club_row(d){
         ...
         if ("about" in d) {
             ...
             unsafe_html.append('<div class="fin-dd-fopen-about">' + d.about + '</div>');
         ...
         if (d["identity_request"] == true) {
             ...
             unsafe_html.append('<div class="ml-5 fin-dd-fopen-about fin-dd-read-more">' + d.identity_mission + '</div>');
             unsafe_html.append('<h5 class="ml-3">Community Building</h5>');
             unsafe_html.append('<div class="ml-5 fin-dd-fopen-about fin-dd-read-more">' + d.identity_community + '</div>');
             unsafe_html.append('<h5 class="ml-3">Supporting Activities/Events</h5>');
             unsafe_html.append('<div class="ml-5 fin-dd-fopen-about fin-dd-read-more">' + d.identity_activity + '</div>');
             unsafe_html.append('<h5 class="ml-3">Inclusivity</h5>');
             unsafe_html.append('<div class="ml-5 fin-dd-fopen-about fin-dd-read-more">' + d.identity_open + '</div>');
             unsafe_html.append('<h5 class="ml-3">Outside Funding</h5>');
             unsafe_html.append('<div class="ml-5 fin-dd-fopen-about fin-dd-read-more">' + d.identity_outsidefunding + '</div>');
         }

         var $mark_as_review_btn = $('<a class="btn btn-success text-light m-1">Mark as reviewed</a>');
         var $remove_from_funding_poll = $('<a class="btn btn-danger text-light m-1">Remove from Funding Poll</a>');
    ...
    static itemiser() {
        ...
        var categories = [
            { text: 'Select...', value: 0, bg_color: '#FFFFFF', text_color: '#000000' },
            { text: 'Refreshments', value: 1, bg_color: '#009688', text_color: '#FFFFFF' },
            { text: 'Administration', value: 2, bg_color: '#3F51B5', text_color: '#FFFFFF' },
            { text: 'Entertainment', value: 3, bg_color: '#9C27B0', text_color: '#FFFFFF' },
            { text: 'Capital Improvements', value: 4, bg_color: '#673AB7', text_color: '#FFFFFF' },
            { text: 'A/V', value: 5, bg_color: '#FF5722', text_color: '#FFFFFF' },
            { text: 'Miscellaneous', value: 6, bg_color: '#E91E63', text_color: '#FFFFFF' }
        ];
        function add_item( i ) {
            ...
            item.append( $('<input/>').attr({ name: 'sin-rf-item-name-'+i, id: 'sin-rf-item-name-'+i, class: "form-control sin-rf-item-name", placeholder: "Item", ariaLabel: "Item" }) );
            category = bootstraped_div(3);
            var $sel = $('<select>').attr({ name: 'sin-rf-choose-category'+i, id: 'sin-rf-choose-category'+i, class: "form-control sin-rf-item-category" });
            $sel.change(function(){
            ...
            var amount_inner_group = $('<div class="input-group mb-3"><div class="input-group-prepend"><span class="input-group-text">$</span></div><input type="number" class="form-control sin-rf-item-cost" placeholder="Amount" step="0.01" aria-label="Amount" name="sin-rf-item-cost-'+i+'" id="sin-rf-item-cost-'+i+'"></div>');
            ...
            deleter.click(function(e){
                e.preventDefault();
                var cost = $(this).parents('.sin-rf-item-row').find('.sin-rf-item-cost').val();
                ...
                var sub = parseFloat($('#sin-rf-item-cost-final').val()) - parseFloat(cost);
                $('#sin-rf-item-cost-final').val( parseFloat(sub).toFixed(2) );
            });

            action.append(deleter);
            var row = $('<div>').attr({ class: 'row sin-rf-item-row', id: 'sin-rf-item-row-'+i });
            row.append( hash ).append( item ).append( category ).append( amount ). append( action );
            return row;
        }
        ...
        item_adder.click(function(e){
            ...
            if ( item_count > 0 ) {
                ...
                $('.input-group').on('input','.sin-rf-item-cost',function(){
                    ...
                    $('#sin-rf-item-cost-final').val( parseFloat( get_total_cost() ).toFixed(2) );
                });
    ...
    static make_senate_position_selector(el, data) {
        ...
        var colorTicker = 0;
        var new_positions = [];
        var badgeColors = ['badge-primary','badge-success','badge-danger','badge-warning','badge-info'];
        $(el).selectize({
            create: true,
            maxItems: 1,
            sortField: 'Role',
            ...
            render: {
                item: function(item, escape) {
                    ...
                    return '<div>' +
                        (item.Role ? '<span class="sin-rfs-name">' + escape(item.Role) + '</span>' : '') +
                        (item.Count || item.Primary ? ` <span class="sin-rfs-email badge ${badgeColor}"> ` + escape(item.Count) + ' in running </span>' : '') +
                    '</div>';
                },
    ...
    static make_signator_selector(el, data) {
        ...
        $(el).selectize({
            ...
            render: {
                item: function(item, escape) {
                    return '<div>' +
                        (item.Name ? '<span class="sin-rc-name">' + escape(item.Name) + '</span>' : '') +
                        (item.Email ? '<span class="sin-rc-email">' + escape(item.Email) + '</span>' : '') +
                    '</div>';
                },
                option: function(item, escape) {
                    ...
                    return '<div>' +
                        ...
                        (caption ? '[<i><span class="caption">' + escape(caption) + '</span></i>]' : '') +
                    '</div>';
                }
            },
    ...
    static show_alert(message,alerttype) {
        ...
        $('#sin-fp-alert-placeholder').append('<div id="alertdiv" class="alert alert-' +  alerttype + '"><a class="close" data-dismiss="alert">×</a><span>'+message+'</span></div>')
    ...
    static generate_club_card( id, title, signators, content, callback_like, callback_dislike, callback_boost ) {

        var card_figure = $('<div class="pmd-card-figure fp-item col d-inline-flex mx-auto"></div>').attr({ id : id });
        var card = $('<div class="pmd-card pmd-card-default pmd-z-depth pmd-card-custom-view"></div>');
        var card_title = $('<div class="pmd-card-title d-flex flex-column"></div>');
        ...
        var dislike_button = $('<button class="btn btn-danger pmd-btn-fab pmd-ripple-effect btn-primary" type="button"><i class="fa fa-thumbs-down"></i></button>');
        var like_button = $('<button class="btn btn-success pmd-btn-fab pmd-ripple-effect btn-primary" type="button"><i class="fa fa-thumbs-up"></i></button>');
        var boost_button = $('<button class="btn btn-warning pmd-btn-fab pmd-ripple-effect btn-primary" type="button"><i class="fa fa-star"></i></button>');
...
class Modules {
    ...
    static ActivateTouchDown() {
        (function(jQuery){
            ...
            jQuery(window).scroll(function() {
                ...
                } else if (!((window.innerHeight + window.pageYOffset) >= document.body.offsetHeight - 60)) {
            ...
            jQuery('.atw-down').css('left', $(window).width()/2 - jQuery('.atw-down').width()/2);

            jQuery(window).on('resize', function(){
                var win = $(this); //this = window
                jQuery('.atw-down').css('left', win.width()/2 - jQuery('.atw-down').width()/2);
            });
    ...
    static ActivateHoverFluid() {
        ...
        var back_colors = ['F44336','E91E63','CE93D8','9575CD','3F51B5','03A9F4','00BCD4','009688','4CAF50','8BC34A','CDDC39','FFEB3B','FFC107','FF9800','FF5722','000000'];
        var fore_colors = ['000000','000000','000000','000000','FFDF00','000000','000000','000000','000000','000000','000000','000000','000000','000000','000000','FFDF00'];
